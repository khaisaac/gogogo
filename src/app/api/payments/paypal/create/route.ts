...
export async function POST(request: Request) {
  try {
    ...
    if (!booking.total_price) {
      return NextResponse.json({ error: "Booking has no price set. Please contact admin." }, { status: 400 });
    }
    ...
    return NextResponse.json({ order_id: paypalOrder.id, approval_url: approvalLink });
  } catch (err) {
