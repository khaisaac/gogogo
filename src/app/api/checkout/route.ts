...
export async function POST(req: Request) {
  try {
    ...
    if (paymentType === "dp") {
      ...
      finalAmountUSD = finalAmountUSD * 0.7;
    }
    ...
    const dokuPayload = {
      order: { invoice_number: invoiceNumber, amount: amountIDR },
      payment: { payment_due_date: 60 },
      customer: { id: customerEmail, name: customerName, email: customerEmail },
    };
    ...
    return NextResponse.json({
      success: true, invoice: invoiceNumber,
      paymentUrl: dokuResponse.data.response.payment.url,
      amountIDR, amountUSD: finalAmountUSD, paymentType,
    });
  } catch (error: any) {
    console.error("Checkout API Error:", error.response?.data || error.message);
    return NextResponse.json({ error: "Failed to process checkout", details: error.response?.data || error.message }, { status: 500 });
  }
}
