...
export default function MultiImageUploadField({
  id, name, currentImages = [], currentImagesFieldName, maxFiles = 10, folder = "packages",
  ...
  return (
    <div className={styles.wrapper}>
      {currentImagesFieldName ? (<input type="hidden" name={currentImagesFieldName} value={mergedImages.join("\n")} />) : null}
      ...
      <input
        ...
        onChange={async (event) => {
          ...
          const remainingSlots = Math.max(0, maxFiles - activeCurrentImages.length - uploadedUrls.length);
          const acceptedFiles = validBySize.slice(0, remainingSlots);

          const messages: string[] = [];
          if (oversized.length > 0) messages.push(`${oversized.length} file(s) skipped: each image must be smaller than 10MB.`);
          if (validBySize.length > acceptedFiles.length) messages.push(`Only ${remainingSlots} new photo(s) accepted.`);
          ...
          try {
            ...
            for (const file of acceptedFiles) {
              ...
              } catch { failedNames.push(file.name || "unknown file"); }
            }
            ...
            if (newUrls.length > 0) statusMessages.push(`${newUrls.length} photo(s) uploaded.`);
            if (failedNames.length > 0) statusMessages.push(`${failedNames.length} photo(s) failed.`);
            setNotice(statusMessages.length > 0 ? statusMessages.join(" ") : "Upload complete.");
          } catch { setNotice("Failed to upload images."); }
          finally {
          ...
          <div className={styles.grid}>
            {activeCurrentImages.map((src, index) => (
              <div key={`${src}-${index}`} className={styles.card}>
                <img src={src} alt={`Current gallery ${index + 1}`} className={styles.img} />
                <button type="button" onClick={() => setRemovedCurrentImages([...removedCurrentImages, src])} className={styles.removeBtn}>Remove image</button>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className={styles.sectionBlock}>
          ...
          <div className={styles.grid}>
            {uploadedUrls.map((src, index) => (
              <div key={`${src}-${index}`} className={styles.card}>
                <img src={src} alt={`Upload preview ${index + 1}`} className={styles.img} />
                <button type="button" onClick={() => setUploadedUrls(uploadedUrls.filter(url => url !== src))} className={styles.removeBtn}>Remove image</button>
              </div>
            ))}
          </div>
          <button type="button" className={styles.cancelBtn} onClick={() => { setUploadedUrls([]); setNotice(null); if (inputRef.current) inputRef.current.value = ""; }}>Cancel all uploads</button>
        </div>
      ) : null}
